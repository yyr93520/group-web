...
from utilClass import Singleton
...
class MongodbClient(object):
	...
	def textSearch(self, keyword):
		return self.db[self.tab].find({"$text":{"$search":keyword}})
	...
	def search(self, prop_vals, search_opera=None):
		"""
		For example: search('uid', weibo_uids, '$in') # get user infos from weibodb
		"""
		if search_opera == None:
			return self.db[self.tab].find(prop_vals)
		return self.db[self.tab].find({search_opera:prop_vals})


	def search_one(self, query):
		if not isinstance(query, dict):
			return None
		return self.db[self.tab].find_one(query)
